# Подключение Supabase

## Что уже подключено (живые ставки)

Клиент сконфигурирован (`src/lib/supabase.ts` — publishable key публичен по дизайну,
защита через Row Level Security). Модуль `src/scripts/live.ts` уже делает:

- **Общие ставки**: при загрузке страницы ставки всех посетителей подтягиваются из
  таблицы `bids` и накладываются на демо-каталог.
- **Realtime**: ставка любого посетителя мгновенно обновляет цену лота у всех, кто
  сейчас на сайте, + тост «Вашу ставку перебили!» тому, кого перебили.
- **Анонимная авторизация**: демо-логин по имени создаёт анонимного auth-пользователя
  Supabase + строку в `profiles` (это требование RLS для записи ставок).

Всё строго опционально: без Supabase (или при ошибке сети/таблиц) каждый вызов
тихо отключается и сайт работает как раньше — на локальных демо-данных.

## Что нужно сделать один раз в Dashboard

1. **SQL Editor → выполнить `supabase/schema.sql`** (таблицы + RLS-политики).
2. **SQL Editor → выполнить `supabase/seed.sql`** (15 демо-лотов — без них ставки
   не запишутся из-за FK, + включение Realtime для `bids`).
3. **Authentication → Sign In / Providers → включить «Allow anonymous sign-ins»** —
   без этого ставки остаются локальными (RLS требует auth.uid()).

После этого откройте сайт в двух браузерах: ставка в одном мгновенно появится во втором.

---

## Дальнейшая миграция (по одной фиче)

1. Создайте проект на https://supabase.com (бесплатный план достаточен для старта).
2. Откройте SQL Editor → вставьте и выполните `supabase/schema.sql` (создаёт таблицы
   `profiles`, `products`, `bids`, `favorites`, `reviews` + RLS-политики).
3. Settings → API → скопируйте **Project URL** и **anon public key**.
4. Скопируйте `.env.example` → `.env`, вставьте туда эти два значения.
5. `src/lib/supabase.ts` теперь экспортирует реальный клиент `supabase` (раньше — `null`).

## Что делать дальше (по одной фиче, не всё сразу)

Самое безопасное — мигрировать по одной точке за раз, тестируя каждую:

- **Авторизация**: заменить `openLogin`/`submitLogin` (`src/scripts/cabinet.ts`) на
  `supabase.auth.signInWithOtp(...)` или `signInWithPassword`.
- **Каталог**: `SEEDED_PRODUCTS` → `await supabase.from('products').select('*')` при инициализации.
- **Ставки**: `placeBid` (`src/scripts/lot-detail.ts`) → `insert` в таблицу `bids` +
  Supabase Realtime подписка для живого обновления у всех зрителей лота.
- **Избранное**: `toggleFavorite` → `insert`/`delete` в таблицу `favorites`.
- **Создание лота**: `publishListing` (`src/scripts/cabinet.ts`) → `insert` в `products`.

Полная схема полей в `supabase/schema.sql` сделана зеркально структуре `Product` в
`src/types.ts` — переносить поля 1:1, без изменения дизайна данных.
